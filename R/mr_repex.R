...
example = function() {
  ...
  parcels = load_parcels(project.dir, c("base","s2r"),just_glob = c("detail","core","regcoef","source","regvar"), ignore_glob = "org_")

  s2r_repex(project.dir, step=51, parcels=parcels)
  navigate_to_repex(project.dir)
}


stata_code_reg_data_preparation = function(project.dir, step, dap=get.project.dap(project.dir, make.if.missing=FALSE), add_reg_code=FALSE) {
  ...
  # 1. Get data file
  step = path$step[1]
  file = mr_get_cache_file(project.dir, step)
  code = paste0(code,'\nuse "', file,'", clear\n')
...
load_parcels = function(project.dir, metaid, just_glob=NULL, ignore_glob="*_header.Rds", parcels=list()) {
  restore.point("load_parcels")
  parcel.dirs = file.path(project.dir,"metareg", metaid, "regdb")
...
navigate_to_repex = function(project.dir, step=NULL, repex.dir = file.path(project.dir,"metareg","s2r",paste0("repex/", step))) {
...
make_repex_project_code = function(project.dir, repex.dir) {
  restore.point("make_repex_project_code")
  new.project.dir = file.path(dirname(project.dir),"_repex")

  first_lines = paste0(
'project.dir = "', new.project.dir,'"
repex.dir = "', repex.dir,'"')
